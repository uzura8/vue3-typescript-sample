...
import MainHeaderUserDropdown from '@/components/molecules/MainHeaderUserDropdown.vue'

export default defineComponent({
  components: {
    MainHeaderUserDropdown
  },
  ...
  setup() {
    ...
    const { email, isAuth } = storeToRefs(userStore)

    const signOut = async () => {
      try {
        globalLoader.updateLoading(true)
        await FirebaseApi.signOut()
        userStore.setMember(null)
