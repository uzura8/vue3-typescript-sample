...
import MemberProfileImageUpload from '@/components/organisms/MemberProfileImageUpload.vue'
...
export default defineComponent({
  ...
  setup() {
    ...
    const { displayName, username, photoURL } = storeToRefs(memberStore)

    const isActiveMemberProfileImageControllModal = ref<boolean>(false)
    const setProfileImage = async (uploadedPhotoURL: string) => {
      try {
        globalLoader.updateLoading(true)
        const updatedMember = await FirebaseApi.updateProfile({ photoURL: uploadedPhotoURL })
      ...
      isActiveMemberProfileImageControllModal.value = false
    }
    const deleteProfileImage = async () => {
      try {
        globalLoader.updateLoading(true)
        const updatedMember = await FirebaseApi.updateProfile({ photoURL: '' })
        setMember(updatedMember)
        globalLoader.updateLoading(false)
      } catch (error) {
        console.error(error)
    ...
    const isActiveMemberProfileImageModal = ref<boolean>(false)
    const openMemberProfileImageModal = () => {
      if (!photoURL.value) return
      isActiveMemberProfileImageModal.value = true
    }
    ...
    return {
      ...
      setProfileImage,
      deleteProfileImage,
      isActiveMemberProfileImageControllModal,
      isActiveMemberProfileImageModal,
      openMemberProfileImageModal
    }
  }
})
</script>
  ...
  <div class="bg-white rounded-lg shadow-xl pb-8">
    ...
    <div class="flex flex-col items-center -mt-20">
      <span class="relative">
        <button
          type="button"
          :class="{ 'cursor-default': !photoURL }"
          @click="openMemberProfileImageModal"
        >
          <img
            v-if="photoURL"
            :src="photoURL"
          ...
          <img
            v-else
            src="/static/img/profile-default.jpg"
            class="w-40 border-4 border-white rounded-full"
          />
        </button>
        <BaseModal
          :is-active="isActiveMemberProfileImageModal"
          @close="isActiveMemberProfileImageModal = false"
        >
          <div class="pb-2">
            <img :src="photoURL" />
          </div>
        ...
        <button
          type="button"
          class="absolute bottom-3 right-3 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white"
          @click="isActiveMemberProfileImageControllModal = true"
        >
          <FontAwesomeIcon
            icon="camera"
        ...
        <BaseModal
          ...
          :is-active="isActiveMemberProfileImageControllModal"
          @close="isActiveMemberProfileImageControllModal = false"
        >
          <MemberProfileImageUpload
            class="sm:min-w-md"
            @uploaded="setProfileImage"
            @deleted="deleteProfileImage"
            @close="isActiveMemberProfileImageControllModal = false"
          />
        </BaseModal>
      </span>
      <div class="flex items-center space-x-2 mt-2">
        <p class="text-2xl">{{ displayName }}</p>
      </div>
